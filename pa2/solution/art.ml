...
(* build: (int*int->int) * int -> Expr 
   Builds an expression tree by using a function rand that generates a
   number from i to j. 
   Implementation: Uses size of depth to determine the next expression.
                   Pattern matches the number generated by rand to an 
                   an expression.
*)

let rec build (rand,depth) = 
	let x = (if depth > 0 then rand (2,7) else rand (0,2)) in
	match x with
	| 0 -> buildX()
	| 1 -> buildY()
	...
	| 6 -> buildThresh((build(rand,0)),(build(rand,0)),(build(rand,depth-1)),(build(rand,depth-1)));;
...
let rec build2 (rand,depth) = 
      let x = (if depth > 0 then rand (2,4) else rand (0,2)) in
       match x with
       | 0 -> buildX()
       | 1 -> buildY()
       | 2 -> Add (build2 (rand,depth-1), build2(rand,depth-1))
       | 3 -> Sub (build2 (rand,depth-1));;
